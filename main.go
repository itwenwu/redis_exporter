...
func main() {
	var (
		...
		inclVerbotenMetrics = flag.Bool("incl-verboten-metrics", getEnvBool("REDIS_EXPORTER_INCL_VERBOTEN_METRICS"), "Whether to include metrics that are not considered kosher by Prometheus standards")
	)
	flag.Parse()
	...
	var hosts []exporter.RedisHost
	switch {
	case *redisFile != "":
		var err error
		if hosts, err = exporter.LoadRedisFile(*redisFile); err != nil {
			log.Fatal(err)
		}
	case *useCfBindings:
		hosts = exporter.GetCloudFoundryRedisBindings()
	default:
		hosts = exporter.LoadRedisArgs(*redisAddr, parsedRedisPassword, *redisAlias, *separator)
	}
